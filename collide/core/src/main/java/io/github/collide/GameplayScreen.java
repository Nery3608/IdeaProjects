...
public class GameplayScreen implements Screen {
    ...
    private void handlePlayerInput(float delta) {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.LEFT) && onGround){
            velocityY = MOVE_SPEED * delta;
            facingRight = false;
        }
        if (Gdx.input.isKeyPressed(Input.Keys.RIGHT) && onGround){
            velocityY = MOVE_SPEED * delta;
            facingRight = true;
        }
    ...
    private void checkCollisions() {
        ...
        for (var enemy : enemies) {
            Rectangle enemyRect = new Rectangle(enemy[0], enemy[1], 40, 36);
            if (playerBounds.overlaps(enemyRect)) {
                playerX = 100;
                playerY = GROUND_Y;
                velocityY = 0;
                System.out.println("Hit! Setting up next player!");
            }
        }
